...
def index():
    ...
    if current_user.is_authenticated:
        ...
        recent_books = [(db.session.query(Book).filter_by(id=book_id).first(), datetime.now())
                        for (book_id,) in recent_book_ids]
    else:
        recent_book_ids = session.get('recent_book_ids', [])
        recent_books = [(book, datetime.now()) for book in (db.session.query(Book).filter_by(id=book_id).first() for book_id in recent_book_ids) if book]
